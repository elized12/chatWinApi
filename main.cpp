...
LRESULT CALLBACK wndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            editMessageComponent = CreateWindow(_T("EDIT"), _T(""), WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL | ES_MULTILINE | ES_READONLY | WS_VSCROLL, 0, 0,
                                                (rectSize.right / 2) - 10, rectSize.bottom, hWnd,
                                                NULL, GetModuleHandle(NULL), NULL);

            editPostMessageComponent = CreateWindow(_T("EDIT"), _T(""), WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL | ES_MULTILINE | WS_VSCROLL, rectSize.right / 2, 0, (rectSize.right / 2), 100,
            ...
            buttonSendMessage = CreateWindow(_T("BUTTON"), _T("���������"), WS_CHILD | WS_VISIBLE | WS_BORDER,
                                             rectSize.right / 2, 105, (rectSize.right / 2) - widthScrollBar, 50, hWnd,
                                             (HMENU)IDB_BUTTON_SEND, GetModuleHandle(NULL), NULL);

            buttonCreateRoom = CreateWindow(_T("BUTTON"), _T("�������"), WS_CHILD | WS_VISIBLE | WS_BORDER,
                                            rectSize.right / 2, 230, (((rectSize.right / 2) - widthScrollBar) / 2) - 5, 25,
                                            hWnd, (HMENU)IBD_BUTTON_CREATE, hInstance, NULL);

            buttonConnectRoom = CreateWindow(_T("BUTTON"), _T("������������"), WS_CHILD | WS_VISIBLE | WS_BORDER,
            ...
            buttonDisconnect = CreateWindow(_T("BUTTON"), _T("��������"),
            ...
            AppendMenu(menu, MF_POPUP, reinterpret_cast<UINT_PTR>(subMenu), _T("������������"));
            AppendMenu(subMenu, MF_STRING, ID_CHANGE_NICKNAME, _T("�������� ���"));
            SetMenu(hWnd, menu);
            ...
            switch (wParam) {
                case IDB_BUTTON_SEND: {
                    ...
                    if (lenghtPostMessage == 0) {
                        MessageBox(NULL, _T("��������� ������"), _T("message"), MB_OK);
                        break;
                    } else {
                        ...
                        if (status.isConnect == false) {
                            ...
                            MessageBox(NULL, _T("�������� �� ����������� ���������� �����"), _T("info"), MB_OK);
                            break;
                        } else {
                ...
                case IBD_BUTTON_CONNECT: {
                    ...
                    if (inet_pton(AF_INET, addressConnectStr.data(), &sockAddress) == 1) {
                        ...
                        if (connectRoom == serverAddres) {
                            MessageBox(hWnd, _T("������ ����������� � ������ ����"), _T("error"), MB_OK);
                            break;
                        }
                        ...
                        if (statusConnect) {
                            ...
                            MessageBox(NULL, _T("����� ������� ����"), _T("information"), MB_OK);
                        }
                    }
                    break;
                }
...
void initServer(Server& server, HWND mainWindow, HWND* editViewMessage, Address& addressServer, int* state) {
    server.setCallback(static_cast<int>(StatusServer::CREATE_ROOM), [mainWindow, editViewMessage, state, addressServer](Connection& conn, thread_safe_queue<Message>& queue, Message& message, ListUser& listUsers) {
        switch (message.type) {
    ...
    server.setCallback(static_cast<int>(StatusServer::JOIN_ROOM), [mainWindow, editViewMessage, state](Connection& conn, thread_safe_queue<Message>& queue, Message& message, ListUser& listUsers) {
        switch (message.type) {
            ...
            case Command::ROOM_IS_NOT_CREATE_NOTIFICATION: {
                *state = 3;
                PostMessage(mainWindow, WM_CHANGE_STATE, NULL, NULL);
                MessageBox(mainWindow, _T("������� ��� �� �������"), _T("info"), MB_OK);
                break;
            }
            default: {
                message.user.addressSocket = Address(TypeAddress::IPV4, conn.getAddress().getIpAddress(), PORT_SERVER);
    ...
    server.setCallback(static_cast<int>(StatusServer::NOT_LISTEN), [mainWindow, editViewMessage, state](Connection& conn, thread_safe_queue<Message>& queue, Message& message, ListUser& listUsers) {
        switch (message.type) {
            case Command::SEND_MESSAGE_TO_SERVER:
            case Command::INIT_USER: {
                message.user.addressSocket = Address(TypeAddress::IPV4, conn.getAddress().getIpAddress(), PORT_SERVER);
...
HWND createWindow(tstring& className, tstring& windowName, HINSTANCE& hInstance, int modeShow) throw(std::exception) {
    HWND window = CreateWindow(className.c_str(), windowName.c_str(), WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 600, 400,
